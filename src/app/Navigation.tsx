    ...
    <>
                    ...
                    <div className="space-y-1">
                        {navItems.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    <Icon className={`w-5 h-5 transition-colors ${isActive ? "text-indigo-400" : "text-gray-500 group-hover:text-white"}`} />
                                    <span className="font-medium text-sm">{item.label}</span>
                                    {isActive && <ChevronRight className="w-4 h-4 ml-auto text-indigo-400 opacity-50" />}
                                </Link>
                            );
                        })}
                    </div>
        ...
        <div className="mt-auto p-4 border-t border-white/5">
           <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 transition-colors cursor-pointer group">
              ...
              <div className="flex flex-col overflow-hidden">
                <span className="text-sm font-semibold text-white truncate">Mauro</span>
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[#0f111a]/80 backdrop-blur-xl border-b border-white/5 z-50 flex items-center justify-between px-4 transition-all duration-300">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-lg text-white">Seerr</span>
        </div>
        
        <Link href="/settings" className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center text-gray-400 border border-white/5 hover:bg-white/10">
        ...
        <div className="grid grid-cols-4 h-16">
          {navItems.map((item) => {
            ...
            return (
                ...
                >
                <div className={`p-1 rounded-lg transition-all ${isActive ? "bg-indigo-500/10" : ""}`}>
