      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="bg-[#161824] border border-white/10 rounded-xl px-4 py-2 text-sm text-gray-400 shadow-lg shadow-black/20">
             <span className="text-indigo-400 font-bold">{requests.length}</span> Total
          </div>
        </div>
      </div>
        ...
        <div className="space-y-12">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 flex flex-col">
                      ...
                      <p className="text-xs text-gray-400 mb-3">
                        Requested by <span className="text-indigo-400 font-medium">{req.requested_by}</span>
                        {req.season && ` • S${req.season} E${req.episode || 'All'}`}
                      </p>
                      
                      <div className="flex gap-2 mt-auto">
                        <button
                          onClick={async () => {
                            await fetch(`/api/request/${req.id}/approve`, { method: "POST" });
                            fetchRequests();
                          }}
                        ...
                        <button
                          onClick={async () => {
                            await fetch(`/api/request/${req.id}/approve`, { method: "DELETE" });
                            fetchRequests();
                          }}
          ...
          {requests.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 text-gray-500 gap-4 border-2 border-dashed border-gray-800 rounded-3xl bg-[#13151f]/50">
              ...
              <p className="text-lg font-medium text-gray-400">No requests yet</p>
              <button
                onClick={() => router.push("/")}
